...
INSERT INTO MUSHROOM (TITLE, IMAGE_URL, ICON, INFO, EDIBLE) VALUES
  ...
  ('Inky Cap', 'mushrooms/inky_cap.png', 'mushrooms/inky-crap-icon.png', 'The Inky Cap typically has a cylindrical or bell-shaped cap that is initially white and then changes to gray and eventually black as it matures. The gills are closely spaced, and the stem is relatively tall and slender. One distinctive feature is the rapid autodigestion of the cap, starting from the edges, which eventually dissolves the entire cap into an inky black liquid. This process gives the mushroom its common name. Inky Caps are often found in disturbed ground, lawns, or wood chip areas. While some species in the Inky Cap group are edible, Coprinus atramentarius should be avoided if consumed with alcohol due to potential adverse reactions.', true),
